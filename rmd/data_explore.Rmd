...
Ok so how can we adjust for this and make a model that doesnt account for games played? Lets remove games played since its unpredictable and try with some advanced stats.

```{r}
advanced_df <- df %>%
  mutate(id = slugPlayerSeason) %>%
  transmute(ratio_per = ratioPER,
...
write_rds(xgb_res, here::here("models/advanced_model.rds"))
```

```{r}
xgb_res <- read_rds(here::here("models/advanced_model.rds"))
...
new_df <- readr::read_csv(here::here("Data/latest_season.csv")) %>%
  dplyr::select(!where(is.factor)) %>%
  rename(fgm_total = fgm_totals, fga_total = fga_totals,
         pct_3p_rate = pct3p_rate, ratio_ws_per_48 = ratio_ws_per48)
...
pred_df %>%
  ...
  ggplot(aes(fct_reorder(name_player, predictions), predictions)) +
  geom_col(color = "#3EB489", alpha = 0.8, fill = "#3EB489") +
  geom_text(aes(label = round(predictions)), hjust = -0.6) +
  coord_flip() +
  labs(x = "Player", y = "Fantasy Points Prediction", title = "Top 50 Predictions Based on Season Totals So Far") +
  theme(legend.position = "none",
        axis.text.y = element_text(size = 8))

ggsave(here::here("images/advanced_predictions.png"), width = 10, height = 10)
```

And what is the importance this time? It looks like field goal makes and point totals show up again, but this time we see vorp make a showing, as well as turnovers and defensive rebounds with some slight performances.
...
ggsave(here::here("images/advanced_importance.png"), width = 10, height = 10)
```

## Linear Model

So all this complicated machine learning stuff is nice but what if we simplified it all, is it really any better than just running an ols regression?

```{r}
lm1 <- lm(fantasy_points ~ ., data = advanced_df)
summary(lm1)

lm2 <- lm(fantasy_points ~ pts_totals + fgm_total + ratio_vorp, data = advanced_df)
summary(lm2)
```
